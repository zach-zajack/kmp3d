module KMP3D
  require "#{PATH}/test/kmp3d_test"
  require "#{PATH}/test/test_import_export"
  require "#{PATH}/test/test_rotations"

  menu = UI.menu.add_submenu("KMP3D Tests")

  rebuild_files = menu.add_submenu("1:1 Rebuild file")
  Dir["#{PATH}/test/kmps/*.kmp"].each do |path|
  ...
  rebuild_all = UI::Command.new("1:1 Rebuild all files") do
    Dir["#{PATH}/test/kmps/*.kmp"].each { |path| TestImportExport.new(path) }
  end
